...
class PriceConverter {
  ...
  static String getCurrency() {
    currencySymbol =  Get.find<DashBoardController>().configModel.content?.currencySymbol ?? "";
  ...
  static String convertPrice(double? price, {double? discount, String? discountType, bool isShowLongPrice = false}) {
    ...
    bool isRightSide = Get.find<DashBoardController>().configModel.content?.currencySymbolPosition == 'right';
    return isShowLongPrice == true ?'${isRightSide ? '' : currencySymbol}'
        '${(price!).toStringAsFixed(int.parse((Get.find<DashBoardController>().configModel.content ?? ConfigContent()).currencyDecimalPoint ?? "1"))
  ...
  static String longToShortPrice(double amount){
    int decimalPoint = int.tryParse(Get.find<DashBoardController>().configModel.content?.currencyDecimalPoint ?? "2") ?? 2 ;
