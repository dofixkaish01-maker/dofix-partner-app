  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<DashBoardController>(
      builder: (controller) {
        ...
        final mainServiceDetails =
            details.where((e) => e.isAddOn == 0).toList();

        final addonServiceDetails =
            controller.bookingDetails?.content?.bookingStatus == 'completed'
                ? dashboardController.savedAddonModelList
                : [];
        ...
        for (final item in addonServiceDetails) {
          final double price =
              double.tryParse(item.serviceCost?.toString() ?? "0") ?? 0;
          final int qty = int.tryParse(item.quantity?.toString() ?? "0") ?? 0;
          itemTotal += price * qty;
                                  ...
                                  children: [
                                    ...mainServiceDetails.map((item) {
                                      ...
                                      return serviceBreakdownRow(
                                        title: item.variantKey?.isNotEmpty ==
                                                true
                                            ? item.variantKey!
                                    ...
                                    ...addonServiceDetails.map((item) {
                                      ...
                                      return serviceBreakdownRow(
                                        title: item.variantKey?.isNotEmpty ==
                                                true
                                            ? "${item.variantKey!} (Addon)"
                                            : "${item.serviceName ?? "Addon Service"} (Addon)",
                                        price: price,
                                        qty: qty,
                                        total: total,
                                      );
                            ...
                            children: [
                              ...
                              if (isAccepted)
                                Expanded(
                                  child: GestureDetector(
                                    onTap: () async {
                                      final lat = booking?.serviceAddress?.lat
                                              ?.toString() ??
                                          "0.0";
                                      final lon = booking?.serviceAddress?.lon
                                              ?.toString() ??
                                          "0.0";
                                ...
                                Expanded(
                                  child: GestureDetector(
                                    onTap: () async {
                                      ...
                                      // debugPrint(
                                      //     "cust phon: ${booking.customer?.phone}");
