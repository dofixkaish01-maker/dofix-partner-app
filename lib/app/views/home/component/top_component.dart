  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<DashBoardController>(builder: (dashController) {
      return GetBuilder<AccountController>(builder: (accountController) {
        final avgRating = accountController
                .providerReviewModel?.content?.rating?.averageRating ??
            double.tryParse(
              (dashController.providerModel.content?.avgRating ?? 0).toString(),
            ) ??
            0.0;

        final reviewCount = accountController
                .providerReviewModel?.content?.rating?.reviewCount ??
            accountController.providerReviews.length;

        final completedCount = dashController
                .bookingCountResponse.content?.bookingsCount?.completed
                ?.toString() ??
            "0";

        final canceledCount = dashController
                .bookingCountResponse.content?.bookingsCount?.canceled
                ?.toString() ??
            "0";

        final totalCount = dashController
                .bookingCountResponse.content?.bookingsCount?.total
                ?.toString() ??
            "0";
        // debugPrint("Top avgRating => $avgRating");
        // debugPrint("Top reviewCount => $reviewCount");
        // debugPrint("providerReviewModel null? => ${accountController.providerReviewModel == null}");
                  ...
                  child: Row(
                    children: [
                      ...
                      InkWell(
                        ...
                        onTap: () {
                          accountController.fetchProviderReviews();
                          // yaha baad me reviews screen open kar dena
                        },
                        child: Padding(
...
//       //     .toList();
//       final completedBookings = controller
//           .bookingCountResponse.content?.bookingsCount?.completed
//           ?.toString() ??
//           "0";
//
//       debugPrint("bookingModel length => ${controller.bookingModel.data?.length}");
//       debugPrint("bookingModelSecond length => ${controller.bookingModelSecond.data?.length}");
//       debugPrint("completed in bookingModel => ${controller.bookingModel.data?.where((b) => (b.bookingStatus ?? "").toString().toLowerCase().trim() == "completed").length}");
//       debugPrint("completed in bookingModelSecond => ${controller.bookingModelSecond.data?.where((b) => (b.bookingStatus ?? "").toString().toLowerCase().trim() == "completed").length}");
//       debugPrint("count API completed => ${controller.bookingCountResponse.content?.bookingsCount?.completed}");
//       for (var b in controller.bookingModel.data ?? []) {
//         debugPrint("bookingModel status => ${b.bookingStatus}");
//       }
//
//       return Padding(
//         padding: const EdgeInsets.only(top: 10.0),
